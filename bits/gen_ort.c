...
int main() {
  for (unsigned pos1 = 0; pos1 < 32; pos1++) {
    for (unsigned pos2 = 0; pos2 < 32; pos2++) {
      printf("unsigned int or_t_%d_%d(unsigned int c, unsigned int a, unsigned int b) {\n", pos1, pos2);
  ...
  for (unsigned pos1 = 0; pos1 < 32; pos1++) {
    for (unsigned pos2 = 0; pos2 < 32; pos2++) {
      ...
      printf("  my_assert(or_t_%u_%u(0x%08xU, 0x%08xU, 0x%08xU) == ", pos1, pos2, c, a, b);
